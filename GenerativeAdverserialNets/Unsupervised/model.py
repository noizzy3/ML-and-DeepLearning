        ...
        for epoch in xrange(F.epoch):
            ...
            for sample_images in batch_iter:
                ...
                _, summary_str, dlossf = self.sess.run(
                    ...
                    feed_dict={self.images: sample_images, self.z: sample_z,
                               global_step: epoch})
                ...
                errD_fake = self.d_loss_fake.eval({self.z: sample_z})
                errD_real = self.d_loss_real.eval({self.images: sample_images})
                errG = self.g_loss.eval({self.z: sample_z})
                errG_actual = self.g_loss_actual.eval({self.z: sample_z})

...
def load_mnist():
    ...
    labels = np.zeros((70000,10),dtype = np.float)
    for l in y:
        labels[l] = 1
    z = zip(X,labels)
    np.random.shuffle(z)
...
def load_cifar():
    def unpickle(file):
        ...
        fo.close()
        return dict
    #data_dir = "~home/subham/DeepLearning/semisupervised_GAN/data/cifar"
    trainX1 = unpickle('data/cifar/data_batch_1')
    trainX2 = unpickle('data/cifar/data_batch_2')
    trainX3 = unpickle('data/cifar/data_batch_3')
    trainX4 = unpickle('data/cifar/data_batch_4')
    trainX5 = unpickle('data/cifar/data_batch_5')

    trainX = np.vstack((trainX1['data'], trainX2['data'], trainX3['data'], trainX4['data'], trainX5['data']))
    trainX_labels = np.concatenate((trainX1['labels'], trainX2['labels'], trainX3['labels'], trainX4['labels'], trainX5['labels']),axis = 0)
    labels = np.zeros((50000,32))
    for x in range(50000):
        labels[x][trainX_labels[x]] = 1
    trainX = np.asarray(trainX / 127.5, dtype=np.float32) - 1.0
    trainX = trainX.reshape(-1, 3, 32, 32)
    print ("...............",len(trainX))
    trainX = trainX.transpose(0, 2, 3, 1)
    data = zip(trainX,labels)
